...
- [ ] XP / Leveling System (#2) — acceptance: XP awarded after every completed game, level displayed on profile and beside username in-game, tier titles unlock at milestones
...
Players earn coins through normal gameplay — a small amount for every completed game, a larger payout for wins, and bonus coins for completing quests and achievements. Coins are spent in a Pack Shop to buy themed booster packs, each containing 3 cards the player does not yet own, with rarity-weighted distribution (Commons most likely, Legendaries rare). Two pack types are available: a same-release pack (more expensive, guarantees all 3 cards from one chosen release) and a random-release pack (cheaper, draws from across all active releases). Once a player owns every card in a given release, that release no longer contributes cards to random packs — only same-release packs for a completed release are blocked from purchase. Opening a pack triggers an animated reveal sequence — cards flip one at a time, with a more dramatic effect for higher-rarity pulls. Packs can only be purchased with earned coins, never real money, keeping the economy fair and competitive. The coin system acts as a soft currency layer that makes every game feel economically meaningful even when you lose — you're always earning toward something — and the choice between focused and random packs adds a light strategic dimension to how you spend.
...
**19. New Releases (R7–R10)**
Expanding the card pool beyond the current 6 releases with 4 more themed sets to reach a total of 10 active releases — potential themes include Pirates, Ancient Egypt, Fantasy/Dragons, and Sports. Further releases (R11+) are held in reserve for later. Each release follows the existing 30-card formula (14 Creature · 8 Item · 6 Action · 2 Event) and introduces two new Event effects to keep the meta evolving. New releases are the most reliable content lever available: they give card collectors new targets, give deck builders new options, shift the competitive meta, and give the whole community something to discuss. The art generation pipeline already exists, making new releases primarily a content and curation effort rather than a technical one.
